...
function addfame($t, d) {
	if (d.Account.Stats.TotalFame == 0) return;
	var $current_fame = $('<span>').addClass('current_fame').addClass('f_tooltip_callout').text(d.Account.Stats.Fame);
	var $total_fame = $('<span>').addClass('total_fame').addClass('f_tooltip').text(d.Account.Stats.TotalFame);
	$total_fame.appendTo($current_fame);	
	$current_fame.appendTo($t);
}
